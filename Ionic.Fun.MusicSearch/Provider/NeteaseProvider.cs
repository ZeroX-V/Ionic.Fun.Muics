...
namespace Ionic.Fun.MusicSearch.Provider
{
    public class NeteaseProvider : IMusicProvider
    {
        ...
        public string getDownloadUrl(Song song)
        {
            ...
            var urlInfO = JObject.Parse(HttpHelper.GET(string.Format(" http://music.163.com/api/song/enhance/player/url?id={0}&ids=%5B{0}%5D&br=3200000", song.id), DEFAULT_CONFIG));
            return  JArray.Parse(urlInfO["data"].ToString())[0]["url"].ToString();
